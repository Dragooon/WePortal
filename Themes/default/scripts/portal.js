...
$(function()
{
	...
	var updateBlocksPos = function()
	{
		...
		$.ajax({
			data: query.join('&'),
			type: 'post',
			url: smf_scripturl + '?action=portal;area=blockupdate'
		});
	};
	...
	$('#wep_modify').click(function()
	{
		if (!$('#wep_modify').data('modifying'))
		{
			...
			$('#wep_modify').data('modifying', true);
		...
		else
		{
			...
			$('#wep_modify').data('modifying', false);
